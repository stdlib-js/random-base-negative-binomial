...
function factory() {
	...
	if ( arguments.length === 0 ) {
		...
		opts = arguments[ 0 ];
		if ( !isObject( opts ) ) {
			throw new TypeError( format( '0PI2h', opts ) );
		}
		if ( hasOwnProp( opts, 'copy' ) && !isBoolean( opts.copy ) ) {
			throw new TypeError( format( '0PI30', 'copy', opts.copy ) );
		}
		if ( hasOwnProp( opts, 'prng' ) ) {
			if ( !isFunction( opts.prng ) ) {
				throw new TypeError( format( '0PI7M', 'prng', opts.prng ) );
			}
			rpois = poisson({
				'prng': opts.prng
			});
			...
			if ( hasOwnProp( opts, 'state' ) && !isUint32Array( opts.state ) ) {
				throw new TypeError( format( '0PI7I', 'state', opts.state ) );
			}
			opts = assign( {}, opts );
			if ( opts.copy === false ) {
				FLG = false;
	...
	*/
	function setState( s ) {
		if ( !isUint32Array( s ) ) {
			throw new TypeError( format( '0PI7J', s ) );
		}
		if ( FLG ) {
